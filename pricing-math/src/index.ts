/**
 * Modelglass feed math — unit-matched price deltas, price-stability analysis,
 * capability diffing, billing-unit warnings, and lifecycle checks.
 *
 * Extracted from switch-check (SCO-193) so the same logic can be shared with
 * the Modelglass MCP server's `compare_models` tool (SCO-217) without a
 * second, independently-drifting implementation. This package has no fetch
 * logic and no CLI presentation of its own — it's pure functions over
 * already-fetched feed data, which is what makes it reusable across a public
 * CLI (switch-check, this repo) and a private API package: both feed it real
 * `GET /v1/models`-shaped objects, structurally compatible with the types
 * below, and get the same math back.
 *
 * Published as `modelglass-router-examples/pricing-math` (root package
 * `exports` subpath) — MIT, same as the rest of this repo (see ../LICENSE).
 */

// ---------------------------------------------------------------------------
// Types — Modelglass feed (the subset these functions read)
// ---------------------------------------------------------------------------
...
/** Plan tiers as returned by `GET /v1/keys` (ADR 0004's pricing-history gate). */
export type PlanTier = "free" | "app" | "starter" | "pro" | "internal";
...
export function historyWindowLabel(tier: PlanTier): string {
  switch (tier) {
    case "free":
 ...
 *  vocabulary, unlike capability *dimensions*, which this module deliberately
